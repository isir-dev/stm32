/*
  
* All rights reserved.
*
* File Name          : CRC_8.c
* Description        :  usnp应用层辅助实现
*
* Version            : V1.1
* Author             : liu Yong
* Date               : 11/20/2012
*
* OriginalVersion    : V1.0
* Original Author    : oylf1986
* Date               : 11/19/2012
* Reference : VSP（使用中文文字输入后下一行再输入代码时需要用英文标点符号结束，不然有红色波浪线）;
*            使用颠倒的直驱表法,其它被注释方法为驱动表法和直驱查表法，还包括一种从文件中读出数据进行校验;
*/

#include <stdio.h>
#include <stdlib.h>
#include "CRC_8.h"

unsigned long int Reflect(unsigned long int ref, char ch);

unsigned char crc_direct_table_8[256] =								 //genPoly = 0x07; 
{     
         
};
/************************************************************************/
/*                                                                      */
/************************************************************************/
unsigned char CRC8_Tab(unsigned char *ucPtr,   unsigned char ucLen)   
{   
    unsigned char ucIndex;                 //   CRC8校验表格索引;  
    unsigned char ucCRC8 =0x00;        //   CRC8字节初始化,驱动表法和直接计算法必须为0x00; 初始化值不同所得结果不同  
    
    while(ucLen--)					//   进行CRC8位校验;   
    {  
        ucIndex	=	ucCRC8^(*ucPtr++);   
        ucCRC8	=	crc_direct_table_8[ucIndex];   
    } 
    return(ucCRC8);   //   返回CRC8校验数据已经取反;  
}     
/************************************************************************/
/*                                                                      */
/************************************************************************/
unsigned char FindCRC(unsigned char *data,unsigned char datalen) //直接计算法计算CRC8;
{
    unsigned char CRC=0x00;   //   CRC8字节初始化,驱动表法和直接计算法必须为0x00; 初始化值不同所得结果不同  
    unsigned char genPoly = 0x07; 
    int j;
    unsigned char i;
    for (i=0; i< datalen; i++)
    {
        CRC ^= *data++;
        for(j = 0; j<8; j++)
        {
            if(CRC & 0x80 )
                CRC = (CRC << 1) ^ genPoly;
            else
                CRC <<= 1;
        }
    }
    CRC &= 0xff;//保证CRC余码输出为2字节。
    return CRC;   //返回CRC8校验数据未取反; 
}
/************************************************************************/
/*                                                                      */
/************************************************************************/
unsigned long int Reflect(unsigned long int ref, char ch) 
{ 
    unsigned long int value=0; 
    int i;
    // 交换bit0 和bit7，bit1 和bit6，类推 
    for(i = 1; i < (ch + 1); i++) 
    { 
        if(ref & 1) 
            value |= 1 << (ch - i); 
        ref >>= 1; 
    } 
    return value; 
} 
